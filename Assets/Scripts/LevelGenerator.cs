...
public class LevelGenerator : MonoBehaviour {
	...
	// maximum number of holes in maze
	public int maxHoles = 4;

	// to keep track of total no. of holes
	private int holes = 0;

	// keep track of maze stages
	...
	void Start () {
		...
		// show text for stage of maze we are in
		stage += 1;
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// place hole with 10% probability 
				// limit the number of holes by maxHoles variable
				if (!mapData[z, x] && (holes < maxHoles) && Random.value < 0.1) {
					holes += 1;
				}
				else {
					// create floor
